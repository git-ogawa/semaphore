...
func (d *SqlDb) CreateProject(project db.Project) (newProject db.Project, err error) {
	...
	insertId, err := d.insert(
		"id",
		"insert into project(name, created, type, alert, alert_chat, alert_slack_url, max_parallel_tasks) values (?, ?, ?, ?, ?, ?, ?)",
		project.Name, project.Created, project.Type, project.Alert, project.AlertChat, project.AlertSlackUrl, project.MaxParallelTasks)
...
func (d *SqlDb) UpdateProject(project db.Project) error {
	_, err := d.exec(
		"update project set name=?, alert=?, alert_chat=?, alert_slack_url=?, max_parallel_tasks=? where id=?",
		project.Name,
		project.Alert,
		project.AlertChat,
		project.AlertSlackUrl,
